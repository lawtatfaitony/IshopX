...
namespace WebApi.AppCode.Utilities
{
    ...
    public class AliyunIP
    {
        ...
        {
            if (queryIP.Length < 8)  // not a internet ip
            {
                return queryIP;
            }
            ...
            for(int i = 0;i<values.Length;i++)
            {
                ...
                if(values[i].Contains("127.0.0.1")|| values[i].Contains("::1")) //
                {
                    return "local";
                }
            } 
             SourceAreaName sourceAreaName = JsonConvert.DeserializeObject<SourceAreaName>(JsonConvert.SerializeObject(JObject.Parse(values[0])));
